...
const ManagerDashboard = () => {
  ...
  const { data: schedules, isLoading: schedulesLoading } = useQuery({
    queryKey: ["schedules"],
    queryFn: () => schedulesApi.getAll({ limit: 9999 }),
  });
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
        {stats.map((stat) => {
          ...
          return (
            <div
              key={stat.title}
              className="bg-white border border-slate-200 rounded-lg p-4 md:p-6 shadow-card"
            >
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div className="order-2 md:order-1">
                  <p className="text-[10px] md:text-sm font-medium text-slate-500 uppercase tracking-wide">
                    {stat.title}
                  </p>
                ...
                <div
                  className={`w-10 h-10 md:w-12 md:h-12 rounded-lg flex items-center justify-center order-1 md:order-2 ${iconColors[stat.color as keyof typeof iconColors]}`}
                >
        ...
        <Card title="Recent Schedules" subtitle="Latest scheduled trips">
          <div className="space-y-2 md:space-y-3">
            {schedules?.data?.slice(0, 5).map((schedule) => (
              <div
                key={schedule.id}
                className="flex items-center justify-between p-3 md:p-4 bg-slate-50 rounded-lg border border-slate-100 gap-2"
              >
                <div className="flex items-center space-x-2 md:space-x-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs md:text-sm text-slate-500 truncate">
                      {schedule.dumpTruck?.plateNumber || "N/A"}
                    </p>
                  </div>
                </div>
